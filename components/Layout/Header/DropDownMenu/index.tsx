    ...
    <Whisper placement='bottomEnd' trigger='click' speaker={RenderMenu}>
      <div className='flex items-center rs-btn bg-transparent'>
        ...
        <div className='relative flex'>
          <img
            src={image}
            className='ml-3 w-[2.5rem] h-[2.5rem] rounded-full bg-white'
          />
          <Badge className='bg-green-light absolute right-0 bottom-1' />
        </div>
    ...
    <Popover ref={ref} className={className} style={{ left, top }} full>
      <Dropdown.Menu onSelect={handleSelect}>
        {dropDownMenu.map((menu, key) => (
          <Dropdown.Item key={key} icon={menu.icon} eventKey={menu.href}>
            {menu.title}
          </Dropdown.Item>
        ))}
      </Dropdown.Menu>
